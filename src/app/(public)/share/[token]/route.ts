 ...
 *   RESUMED → reprise d'une session déjà ouverte (même runId).
 *   READONLY → le jeton est COMPLETED : projet affiché en lecture seule.
...
export async function GET(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const reason = result.kind // 'invalid' | 'revoked' | 'expired'
  return NextResponse.redirect(
